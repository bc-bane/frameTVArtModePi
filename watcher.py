#!/usr/bin/env python3
"""
Apple TV → Samsung Frame Art Mode watcher

- Watches Apple TV power state via pyatv CLI (`atvremote`).
- When Apple TV turns OFF, waits a few seconds, then
  tries to restore the Frame TV to Art Mode.
- Uses fresh SamsungTVWS connections each time to avoid stale sockets.
"""

import sys
import time
import subprocess

from samsungtvws import SamsungTVWS
import urllib3

urllib3.disable_warnings()

# === USER CONFIG ======================================================

# Replace Frame TV IP address (static IP recommended)
FRAME_IP = "192.168.68.XX"

# Path to the token file you created when pairing with the Frame
TOKEN_FILE = "/home/youruser/frame_token.txt"

# Apple TV identifier from `atvremote ... wizard`
ATV_ID = "YOUR-APPLE-TV-ID"

# How long to wait after Apple TV turns off before touching the Frame (seconds)
WAIT_AFTER_ATV_OFF = 5

# ======================================================================


def get_tv():
    """
    Create a *fresh* SamsungTVWS instance each time.

    This avoids issues where a long-lived connection goes stale and
    starts throwing `[Errno 113] No route to host` even though the TV
    is reachable again.
    """
    return SamsungTVWS(host=FRAME_IP, port=8002, token_file=TOKEN_FILE)


def atv_on():
    """
    Check if Apple TV is on using pyatv's CLI (atvremote).

    Returns:
        True if power state is "On"
        False if "Off" / "Standby" / unknown after a couple of retries
    """
    retries = 0
    while True:
        try:
            out = subprocess.check_output(
                [
                    sys.executable,
                    "-m",
                    "pyatv.scripts.atvremote",
                    "--id",
                    ATV_ID,
                    "power_state",
                ],
                text=True,
            ).strip()
        except subprocess.CalledProcessError:
            out = ""

        if out:
            # Handles formats like "PowerState.On" or "PowerState: On"
            if "On" in out:
                return True
            if "Off" in out or "Standby" in out:
                return False

        retries += 1
        if retries > 1:
            print(f"tv power_state error, retrying ({retries}) -> got: {out!r}")
            time.sleep(1)


def tv_status():
    """
    Return Frame TV power state string, e.g. 'on', 'standby', 'unknown'.
    """
    try:
        tv = get_tv()
        info = tv.rest_device_info()
        return info["device"].get("PowerState", "unknown").lower()
    except Exception as e:
        print(f"Error getting TV status: {e}")
        return "unknown"


def art_mode_is_on(max_retries=20):
    """
    Check if Frame is currently in Art Mode.

    Returns:
        True if art mode is on
        False if explicitly off, or if retries are exhausted.

    max_retries is mainly to avoid infinite loops if the TV vanishes
    from the network or keeps sending non-standard messages.
    """
    retries = 0
    while retries < max_retries:
        try:
            tv = get_tv()
            mode = tv.art().get_artmode()
            if mode in ("on", "off"):
                return mode == "on"
        except Exception as e:
            # Samsung sometimes sends event payloads instead of simple on/off,
            # and sometimes the TV is simply unreachable.
            print(f"Error getting art mode: {e}")

        retries += 1
        if retries > 1:
            print(f"Art mode check retry {retries}, TV status: {tv_status()}")
        time.sleep(1)

    print("Art mode check gave up after max retries.")
    return False


def tv_power_toggle():
    """
    Toggle TV power / Art mode via shortcuts().power().
    """
    try:
        tv = get_tv()
        tv.shortcuts().power()
    except Exception as e:
        print(f"Error toggling TV power: {e}")


def main():
    print("Frame watcher started.")
    atv_has_been_on = False
    tv_has_been_sleeping = False

    while True:
        atv_state = atv_on()
        status = tv_status()

        # Apple TV is ON and Frame is not in deep standby
        if atv_state and status != "standby":
            if not atv_has_been_on:
                print("tv turned ON")
            atv_has_been_on = True
            tv_has_been_sleeping = False

            # Poll more frequently when ATV is on (more responsive)
            time.sleep(0.5)

        # Apple TV is OFF or in standby
        elif not atv_state:
            # If we're already in art mode, just chill
            if art_mode_is_on(max_retries=5):
                time.sleep(2)
            else:
                if atv_has_been_on:
                    print("tv turned OFF → restoring Art Mode")
                    atv_has_been_on = False

                    # Give the Frame time to finish HDMI/CEC power-down
                    print(f"  - Waiting {WAIT_AFTER_ATV_OFF} seconds for Frame to settle...")
                    time.sleep(WAIT_AFTER_ATV_OFF)

                    # Try up to 5 times to make sure Art Mode actually sticks
                    for attempt in range(5):
                        print(f"  - Attempt {attempt + 1}: toggling power & checking Art Mode")
                        tv_power_toggle()
                        time.sleep(3)

                        if art_mode_is_on(max_retries=5):
                            print("✅ Art mode successfully enabled.")
                            break

                        print("  - Art mode still off, waiting before retry...")
                        time.sleep(3)
                    else:
                        print("❌ Gave up trying to restore Art Mode after 5 attempts")
                else:
                    # Apple TV might be on again but Frame in standby
                    if atv_on() and status == "standby":
                        print("tv ON but Frame in standby → waking Frame")
                        tv_power_toggle()
                        time.sleep(2)
                    else:
                        if not tv_has_been_sleeping:
                            print("Frame appears to be sleeping")
                        tv_has_been_sleeping = True
                        # ATV off and Frame sleeping: check less often
                        time.sleep(2)

        # Fallback: should rarely hit here, but don't spin hot
        else:
            time.sleep(1)


if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
        print("\nStopping watcher.")
